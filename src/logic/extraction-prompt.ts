...
export function buildExtractionPrompt(
        ...
        propositions: [
          {
            ...
            predicate: "...",
            truth: true,
            source: "narrator | claim_by_character | claim_by_record | lore | narrative",
            location: "...",
          },
        ],
        rules: [
  ...
  return [
    ...
    "- ATTRIBUTE THE SOURCE. Set the optional `source` field to indicate WHO is asserting the proposition in the story:",
    '  - "narrator" — the narrator/authorial voice presents it as fact.',
    '  - "claim_by_character" — a character says, believes, or testifies it.',
    '  - "claim_by_record" — an in-story document, record, news report, or transcript states it.',
    '  - "lore" — established by the author-provided lore database.',
    '  - "narrative" — default/uncertain (omit the field if unsure).',
    '  This matters for "contested truth" plot devices: if a character claims "I am innocent" but a court record claims "guilty", give them DIFFERENT sources so the verifier knows it is a deliberate disagreement, not a logic bug.',
    ...
    '- For `source`, use "lore" for author-provided lore, "paragraph N" for narrative-introduced rules, or "common_sense" for universally true real-world physics/biology that the narrative does not need to re-evidence inline (e.g., "physical trauma causes lasting impairment", "fire requires fuel and oxygen", "objects fall when dropped"). The verifier will trust common-sense rules without demanding inline justification.',
    "",
