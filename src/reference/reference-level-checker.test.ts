...
describe("applyReferenceLevel", () => {
  ...
  //#when level is 4
  //#then lore_coverage is promoted to error
  it("level 4 promotes lore_coverage to error", () => {
    const results: CheckResult[] = [
      makeResult({ rule: "lore_coverage", severity: "warning" }),
    ];
  ...
  //#when level is 5
  //#then same promotions as level 4 apply
  it("level 5 behaves same as level 4 for severity promotion", () => {
    const results: CheckResult[] = [
      makeResult({ rule: "unsourced_critical", severity: "warning" }),
      makeResult({ rule: "vague_history", severity: "warning" }),
      makeResult({ rule: "lore_coverage", severity: "warning" }),
    ...
    expect(filtered[2].severity).toBe("error");
    expect(filtered[3].severity).toBe("warning");
  });
});
